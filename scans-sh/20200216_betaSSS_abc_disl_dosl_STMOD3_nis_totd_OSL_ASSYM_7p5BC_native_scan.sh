...
#
# screen -d -m ./20200216_betaSSS_abc_disl_dosl_STMOD3_nis_totd_OSL_ASSYM_7p5BC_native_scan.sh > 20200216_betaSSS_abc_disl_dosl_STMOD3_nis_totd_OSL_ASSYM_7p5BC_native_scan.log


# use ASSYM nis_totd and lateral sizes
# corrected nis_totd, old ones were calculated with only 1*dtot
...
# disl=8:4:20, dosl=20:4:52 with 24<=dtot<=60
...
jobname="SSS_180x360_P${disl_str2}_${dosl_str2}_STMOD3_nis_totd_N50_OSL_ASSYM_7p5BC_native_v2corr"
echo $jobname
...
echo "" >> $output
echo "$XNDiff -l -openmp +o $jobname -X +cif betaSSS_abc.cif cif_core.dic -sym symop_betaSSS_abc.txt -nis 0.490371 0.170097 0.300522 0.034991 0.004019 0.000000 +av 0 0.0 90.0 0.5 0.0 360.0 1.0 -sin -init_n_rand -10112 -conc def -distr +par $disl_str $dosl_str 310.0 350.0 333.0 +f stackcpp 400.0 100.0 120.0 40.0 0.0 0.0 0.0 50 0.0 20.0 0.0 0.0 0 2 3 0.001 0.45 450 50 0 0 1 0 0 1 6 5 -silent" >> $output
echo "" >> $output
echo "cd out/" >> $output
